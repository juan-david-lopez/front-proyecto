...
export function useReceipts(options: UseReceiptsOptions = {}): UseReceiptsReturn {
  ...
  const searchReceipts = useCallback(async (
    ...
    try {
      ...
      const effectiveUserId = userId || searchFilters.userId;
      if (!effectiveUserId) {
        throw new Error('userId is required for searching receipts');
      }
      
      const results = await receiptService.getUserReceipts(effectiveUserId, searchFilters);
